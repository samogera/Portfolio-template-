...
const categories: { icon: IconType; name: string }[] = [
...
export default function SkillsStore() {
  ...
  function getCombinedData(): SkillCategory {
    return (skillsData as any)[selectedCategory] as SkillCategory;
  }
    ...
    <div className="flex flex-col md:flex-row bg-white text-black font-sans rounded-2xl w-full max-w-6xl h-[700px] overflow-hidden border-gray-300 border-[1px]">
      ...
      <aside className="hidden md:flex flex-col w-60 bg-gray-100 p-4 gap-2">
        ...
        <div className="relative w-full mb-4">
          {(() => {
            const SearchIcon = FiSearch as unknown as React.ComponentType<{ className?: string }>;
            return (
        ...
        {categories.map(({ icon, name }) => {
          const IconComp = icon as unknown as React.ComponentType<{
            className?: string;
          }>;
          return (
          <div
          ...
          >
            <div className="text-lg text-blue-400"><IconComp /></div>
      ...
      <main className="hide-scrollbar flex-1 p-6 md:p-12 overflow-y-auto space-y-10">
            ...
            >
              {searchFilter(filteredData.featured).map((item, idx) => (
                ...
                >
                  ...
                  {item?.image ? (
                    <div className="h-44 w-full mt-auto rounded-xl overflow-hidden flex justify-center items-center bg-zinc-200">
                      <Image
                        src={item.image}
                        alt={item.name}
                        width={176}
        ...
        {searchFilter(filteredData.learning)?.length > 0 && (
          <div className="space-y-4">
            <h2 className="text-lg font-medium">Installing / Currently Learning</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
